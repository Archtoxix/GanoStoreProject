...
from .models import Comment, Complaint, User, Program, Rating, Category, Genre
...
def program_detail(request, program_id):
    ...
    if request.method == 'POST':
        if request.user.is_authenticated and not request.user.is_blocked:
            ...
            return HttpResponseForbidden("Ви не маєте права оцінювати програми.")
    else:
        form = RatingForm()

        if request.user == program.uploaded_by or request.user.is_staff or request.user.is_superuser:
            edit_form = ProgramUploadForm(instance=program)
        else:
            edit_form = None
...
def submit_complaint(request, program_id):
    ...
    if request.user.is_blocked:
        return JsonResponse({'error': "Ваш обліковий запис заблоковано. Ви не можете подавати скарги."}, status=403)
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            complaint.save()
            return JsonResponse({'success': "Вашу скаргу успішно подано."})
        else:
            return JsonResponse({'error': form.errors}, status=400)
    else:
        return JsonResponse({'error': "Невірний тип запиту."}, status=400)
...
def home(request):
    ...
    if category_id:
        programs = programs.filter(category__id=category_id)
    if genre_id:
        programs = programs.filter(genre__id=genre_id)
...
def delete_program(request, program_id):
    program = Program.objects.get(id=program_id)
    if request.user != program.uploaded_by and not request.user.is_superuser and not request.user.is_staff and not request.user.is_blocked:
        return HttpResponseForbidden()
    program.delete()
    return redirect('home')
...
def secret_question(request):
    ...
    user = User.objects.get(id=user_id)
    if request.method == 'POST':
        form = SecretQuestionAnswerForm(request.POST)
        if form.is_valid():
            if form.cleaned_data['answer'] == user.secret_answer:
                return redirect('set_new_password')
            else:
                messages.error(request, 'Відповідь неправильна. Спробуйте ще раз.')
                return redirect('secret_question')
    else:
        form = SecretQuestionAnswerForm()
